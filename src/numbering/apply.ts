...
export function applyNumbering(
  ...
  visit(ast, (node: Node, index: number | undefined, parent: Node | undefined) => {
    ...
    if (node.type === 'html') {
      ...
      const value = (htmlNode as any).value || ''
      if (/^\s*<!--\s*mmn:/.test(value)) {
        ;(htmlNode as any)._mmnDirective = true
      }
      return
    }
    ...
    const excludedParentTypes = ['blockquote', 'list', 'listItem', 'table', 'tableRow', 'tableCell']
    ...
    if (node.type === 'heading') {
      ...
      if (heading.children.length > 0 && heading.children[0].type === 'text') {
        ;(heading.children[0] as Text).value = prefix + (heading.children[0] as Text).value
      } else {
    ...
    if (node.type === 'paragraph' && state.scope === 'heading+paragraph') {
      ...
      // Skip paragraphs that look like tables
      if (paragraph.children.length > 0 && paragraph.children[0].type === 'text') {
        const textValue = (paragraph.children[0] as Text).value.trim()
        if (textValue.startsWith('|') && textValue.includes('|')) {
          return
        }
        // Skip math blocks ($$...$$)
        if (textValue.startsWith('$$') && textValue.includes('$$')) {
      ...
      if (paragraph.children.length > 0 && paragraph.children[0].type === 'text') {
        ;(paragraph.children[0] as Text).value = prefix + (paragraph.children[0] as Text).value
      } else {
